...
// ------------------------------------------------------------------
// REQUIRE AUTH: Blocks unauthenticated users & checks for bans
// ------------------------------------------------------------------
export async function requireAuth(req, res, next) {
  ...
  try {
    ...
    req.user = decoded; // Payload usually contains { id, email, username }
    next();
  } catch (err) {
    return res.status(401).json({ message: "Invalid or expired token." });
  }
...
// ------------------------------------------------------------------
// OPTIONAL AUTH: Identifies users if logged in, but doesn't block
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
// REQUIRE ADMIN: Strictly for admin-only routes (Dashboard, etc.)
// ------------------------------------------------------------------
